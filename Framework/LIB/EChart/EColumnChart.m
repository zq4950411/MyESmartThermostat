...
//            NSInteger totalColumns = [_dataSource numberOfColumnsInEColumnChart:self];
            ...
            for (int i = 0; i < 11; i++)
            {
                ...
                eColumnChartLabel.text = [[NSString stringWithFormat:@"%i ", (int)valueGap * (10 - i)] stringByAppendingString:[_dataSource highestValueEColumnChart:self].unit];;
...
- (void)moveLeft //这里可以设置翻页的个数
{
    ...
    NSInteger index = _leftMostIndex + 6 * DIRECTION;
    EColumnDataModel *eColumnDataModel = [_dataSource eColumnChart:self valueForIndex:index];
    if (nil == eColumnDataModel) return;
    
    _leftMostIndex = _leftMostIndex + 6 * DIRECTION;
    _rightMostIndex = _rightMostIndex + 6 * DIRECTION;
    ...
    for (int i = 0; i < totalColumnsRequired; i++)
    {
        ...
        EColumn *nextEColumn = [_eColumns objectForKey:[NSNumber numberWithInteger:_leftMostIndex - (i + 6)  * DIRECTION]];
        EColumnChartLabel *eColumnChartLabel = [_eLabels objectForKey:[NSNumber numberWithInteger:_leftMostIndex - i * DIRECTION]];
        EColumnChartLabel *nextEColumnChartLabel = [_eLabels objectForKey:[NSNumber numberWithInteger:_leftMostIndex - (i + 6) * DIRECTION]];
...
- (void)moveRight
{
    ...
    NSInteger index = _rightMostIndex - 6 * DIRECTION;
    EColumnDataModel *eColumnDataModel = [_dataSource eColumnChart:self valueForIndex:index];
    if (nil == eColumnDataModel) return;
    ...
    for (int i = 0; i < totalColumnsRequired; i++)
    {
        ...
        EColumn *nextEColumn = [_eColumns objectForKey:[NSNumber numberWithInteger:_rightMostIndex + (i + 6) * DIRECTION]];
        EColumnChartLabel *eColumnChartLabel = [_eLabels objectForKey:[NSNumber numberWithInteger:_rightMostIndex + i * DIRECTION]];
        EColumnChartLabel *nextEColumnChartLabel = [_eLabels objectForKey:[NSNumber numberWithInteger:_rightMostIndex + (i + 6) * DIRECTION]];
