...
- (void)valueChanged
{
    ...
    if ([self.selectedIndexes containsIndex:tappedSegementIndex]) {
        ...
        if (self.mydelegate) {
            [self.mydelegate multiSelect:self didChangeValue:NO atIndex:tappedSegementIndex];
        }
    }
    else {
        [self.selectedIndexes addIndex:tappedSegementIndex];
        if (self.mydelegate) {
            [self.mydelegate multiSelect:self didChangeValue:YES atIndex:tappedSegementIndex];
        }
    }
    [self selectSegmentsOfSelectedIndexes];
}
