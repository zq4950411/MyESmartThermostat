...
{
    if ([u rangeOfString:URL_FOR_SOCKET_SAVE_PLUG_SCHEDULE].location != NSNotFound)
    {
        if ([jsonString isKindOfClass:[NSString class]])
        {
            if ([jsonString isEqualToString:@"OK"])
            {
                PlugControlViewController *plug = (PlugControlViewController *)[UIUtils getControllerFromNavViewController:self andClass:[PlugControlViewController class]];
                [plug sendGetDatas];
    ...
    for (int i = 0; i < self.datas.count; i++)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                NSNumber *i2 = [NSNumber numberWithInt:[self timeToInt2:etid]];
...
//    }
    ...
    for (int i = 0; i < self.datas.count; i++)
    {
        ...
        else
        {
            ...
            etid = [self timeToInt2:[array objectAtIndex:1]];
        }
        
        [sb appendString:@"{"];
        [sb appendFormat:@"\"stid\":%d,\"etid\":%d}",stid,etid];
    }
    [sb appendString:@"],\"weekDays\":["];
    ...
    [params setObject:sb forKey:@"schedules"];
    if (self.schedule == nil)
    {
        [params setObject:@"-1" forKey:@"scheduleId"];
        [params setObject:@"addSchedule" forKey:@"action"];
    }
    else
    {
        [params setObject:schedule.scheduleId forKey:@"scheduleId"];
        [params setObject:@"editSchedule" forKey:@"action"];
    }
...
-(void) datePickValueStid:(NSString *) stid etid:(NSString *) etid andTag:(int)tag
{
    ...
    NSNumber *n2 = [NSNumber numberWithInt:[self timeToInt2:etid]];
...
- (BOOL)textFieldShouldBeginEditing:(UITextField *)textField
{
    ...
    if ([dic isKindOfClass:[NSDictionary class]])
    {
        ...
        NSString *etid = [self intToString2:[[dic objectForKey:@"etid"] intValue]];
...
-(UIView *) tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    if (section == 0 || section == 1)
    {
        ...
        {
            label.text = @"Periods when the smart plug is On ";
        }
...
-(UITableViewCell *) tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section == 0)
    {
        if (weekCell == nil)
        {
            ...
            self.weekCell.object = self.schedule.weekdays;
        }
        ...
        if (self.weekCell.selectedArray == nil)
        {
            ...
            PlugControlViewController *plug = (PlugControlViewController *)[UIUtils getControllerFromNavViewController:self andClass:[PlugControlViewController class]];
            for (int i = 0; i < plug.datas.count; i++)
            {
                ScheduleEntity *s = [plug.datas safeObjectAtIndex:i];
                if ([s isKindOfClass:[ScheduleEntity class]])
                {
                    if (![s isEqual:self.schedule])
                    {
                        [selecteArray addObjectsFromArray:s.weekdays];
                    }
                }
            }
...
- (void)viewDidLoad
{
    ...
    {
        self.datas = [NSMutableArray arrayWithArray:self.schedule.periods];
        currentAddIndex = -1;
    }
}
