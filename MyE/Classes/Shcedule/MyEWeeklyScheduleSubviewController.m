		//
//  MyEWeeklyScheduleSubviewController.m
//  MyE
//
...
@interface MyEWeeklyScheduleSubviewController(PrivateMethods)
...
@implementation MyEWeeklyScheduleSubviewController
...
@synthesize addNewModeButton = _addNewModeButton;
@synthesize editModeButton = _editModeButton;
@synthesize weeklyModel = _weeklyModel;
@synthesize weeklyModelCache = _weeklyModelCache;
...
- (void)viewDidLoad
{
    ...
    [self setIsRemoteControl:_isRemoteControl];// 重新调用一次，因为有可能在外部第一次设置isRemoteControl]的时候，调用下面的setIsRemoteControl:函数，但那时候View组件还没加载生成完成，那么就可能不能正确设置subviews的可见性。
...
@implementation MyEWeeklyScheduleSubviewController(PrivateMethods)
