//
//  MyEBlockAlertViewWithCheckbox.m
//  MyE
//
...
@implementation MyEBlockAlertViewWithCheckbox
...
+ (void)initialize
{
    if (self == [MyEBlockAlertViewWithCheckbox class])
    {
...
{
    return [[[MyEBlockAlertViewWithCheckbox alloc] initWithTitle:title message:message] autorelease];
}
...
- (id)initWithTitle:(NSString *)title message:(NSString *)message 
{
    if ((self = [super init]))
    {
        ...
        if (message)
        {
            ...
            labelView.backgroundColor = [UIColor clearColor];
            labelView.textAlignment = UITextAlignmentLeft;//UITextAlignmentCenter;//Modified by YY @ 2012-06-26
...
///////////////////////////////////////////////////////////////////////////////////////////////////
#pragma mark - Public added by YY
//* start added by YY
- (BOOL) isChecked {
...
- (void) addCheckboxWithTitle:(NSString *)title {
    ...
    frame.origin.x = floorf((frame.size.width - background.size.width) * 0.5);
    frame.size.width = background.size.width;
    CGSize size = CGSizeMake(frame.size.width-kAlertViewBorder*2, kAlertButtonHeight);
    
    _checkbox = [[MyECheckBoxView alloc] initWithFrame:CGRectMake(kAlertViewBorder, _height, frame.size.width-kAlertViewBorder*2, size.height)];
    _checkbox.label.text = @"I knew. Don't show again.";
    [_checkbox setBackgroundColor:[UIColor clearColor] showBorder:YES  showShadow:NO];
    [_view addSubview:_checkbox];
...
- (void)buttonClicked:(id)sender 
{
    ...
    [self dismissWithClickedButtonIndex:buttonIndex animated:_isHideAnimated];// changed by YY
}
