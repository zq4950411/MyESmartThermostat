//
//  MyEStaycationDetailViewController.m
//  MyE
//
...
@interface MyEStaycationDetailViewController ()
- (void)configureView;
...
@implementation MyEStaycationDetailViewController
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([[segue identifier] isEqualToString:@"ShowVacationEditFromStaycation"]) {
        MyEVacationEditFromStaycationViewController *vacationViewController = [segue destinationViewController];
        vacationViewController.delegate = (id <MyEVacationDetailViewControllerDelegate>) self.delegate;
        vacationViewController.editType = self.editType;
...
//        vacationItem.returnDateTime = [self.dateTimeFormatter dateFromString:[NSString stringWithFormat:@"%@ %@",self.endDateTextField.text, self.sleepTimeTextField.text]];
        ...
        // 现在不需要把Staycation的setpoints传递到vacation了，所以注释了下面的代码
...
- (void) setEditable:(BOOL)editable {
    ...
    self.startDateTextField.enabled = editable;
    self.startDateTextField.textColor = editable ? [UIColor blackColor] : [UIColor grayColor];
    
    self.riseTimeTextField.enabled = editable;
    self.riseTimeTextField.textColor = editable ? [UIColor blackColor] : [UIColor grayColor];
    
    self.endDateTextField.enabled = editable;
    self.endDateTextField.textColor = editable ? [UIColor blackColor] : [UIColor grayColor];
    
    self.sleepTimeTextField.enabled = editable;
    self.sleepTimeTextField.textColor = editable ? [UIColor blackColor] : [UIColor grayColor];
    
    self.dayHeatingTextField.enabled = editable;
    self.dayHeatingTextField.textColor = editable ? [UIColor blackColor] : [UIColor grayColor];
    
    self.dayCoolingTextField.enabled = editable;
    self.dayCoolingTextField.textColor = editable ? [UIColor blackColor] : [UIColor grayColor];
    
    self.nightHeatingTextField.enabled = editable;
    self.nightHeatingTextField.textColor = editable ? [UIColor blackColor] : [UIColor grayColor];
    
    self.nightCoolingTextField.enabled = editable;
    self.nightCoolingTextField.textColor = editable ? [UIColor blackColor] : [UIColor grayColor];
...
-(void)alertView:(UIAlertView *)alertView  clickedButtonAtIndex:(int)index
{
    if([alertView.title isEqualToString:@"Delete"] && index == 0) {
        ...
        if ([self.delegate respondsToSelector:@selector(didFinishEditStaycation:editType:)]) {
            [self.delegate didFinishEditStaycation:self.staycationItem editType:2];//editType：0-修改， 1-新增， 2-删除
        }
    }
}
#pragma mark -
...
- (IBAction)switchTypeToVacation:(id)sender {
    ...
    [self performSegueWithIdentifier:@"ShowVacationEditFromStaycation" sender:sender];
}
@end
